...
const Prayer = ({address, id, prayer, timestamp}) => {
    ...
    useEffect( 
        () => 
        setHasLiked(
            likes.findIndex((like) => like.id === userAddress || session?.user?.email) !== -1
        ),
    [likes])
    ...
    const likepost = async () => {
        ...
        if(hasliked) {
            ...
            await setDoc(doc(db, 'Prayers', id, 'likes', userAddress || session?.user?.email), {
    ...
    <div className={styles.listContainer}>
        ...
        <div className='flex flex-row items-center ml-2 relative top-[34px]'>
            {isAuthenticated && hasliked ?
                <BsHeartFill className={`cursor-pointer text-[#f00] ${!isAuthenticated || !session && 'cursor-not-allowed'}`} disabled={!isAuthenticated || !session} onClick={likepost} size={30} />    
                :
                <BsHeart disabled={!isAuthenticated || !session} onClick={likepost} className={`${!isAuthenticated || !session ? 'cursor-not-allowed' : 'hover:scale-125 transition-all duration-150 ease-out cursor-pointer'}`} size={30} /> 
            }
