    ...
    <div className={styles.listContainer}>
        ...
        <div className='flex flex-col max-w-full max-h-full gap-[5px] '>
            ...
            <div className='flex items-center p-1'>
                ...
                <input disabled={!session } required maxLength={60} minLength={1} type='text' value={comment} onChange={(e) => {setComment(e.target?.value)}} placeholder={` ${session ? 'Add a comment...' : 'login to comment'}`} className='bg-[#e4e6e8] p-2 w-full border-none flex-1 focus:ring-0 outline-none' />
                <button type='submit' disabled={!comment?.trim()} onClick={sendComment} className={`${!comment || !session ?  'bg-[#888] text-[#fff] cursor-not-allowed' : ' text-white bg-[#16bafb] cursor-pointer transition-all ease-out duration-300 hover:scale-105' } rounded-[50%] p-[5px] flex  `}>
